		...
		for (int i = 0; i < wrl.size(); i++)
		{
			...
			if (parts.length >= 1 && parts[0].equals("Shape")) //found a new object
			{
				...
				for (int n = 0; n < coord.size(); n+=3)
				{
					if (flipRB)
					{
						v.add("v "+coord.get(n)+" "+coord.get(n+1)+" "+coord.get(n+2)+" "+color.get(n+2)+" "+color.get(n+1)+" "+color.get(n)); //swap red and blue
					}
					else
					{
						v.add("v "+coord.get(n)+" "+coord.get(n+1)+" "+coord.get(n+2)+" "+color.get(n)+" "+color.get(n+1)+" "+color.get(n+2));
					}
				}
